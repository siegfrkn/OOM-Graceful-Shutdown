...
static bool is_graceful_shutdown(int pid){
        char *file_path = "/proc/graceful_shutdown";
        struct file *f;
        f = filp_open(file_path, O_RDONLY, 0);
        if (IS_ERR(f)){
                printk(KERN_ERR "%s opened.\n", file_path);
        }
        char *buf;
        buf = kmalloc(80, GFP_KERNEL);
        int len;
        len = kernel_read(f, buf, 80 - 1, &f->f_pos);
        ...
        while(token != NULL){
                int read_pid;
                kstrtoint(token, 10, &read_pid);
                printk(KERN_INFO "PID: %s\n", token);
                if (read_pid == pid){
                        printk(KERN_INFO "Process %d marked for graceful shutdown.\n", pid);
                        kfree(buf);
                        return true;
                }
                token = strsep(&buf, ",");
...
static int oom_evaluate_task(struct task_struct *task, void *arg)
{
	...
	int pid;
	pid = (int)task->pid;
	printk(KERN_INFO "Inside OOM_KILL, PID : %d", pid);
	
	if (is_graceful_shutdown(pid))
